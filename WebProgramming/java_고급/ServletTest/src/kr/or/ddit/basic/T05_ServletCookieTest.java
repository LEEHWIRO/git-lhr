 ...
 *  - 쿠키 전송단계 : 웹 브라우저는 저장한 쿠키를 요청이 있을때마다 웹서버에 전송한다.
 *  		(삭제되기 전까지...) 
...
//		 readCookieExam(req, resp);
	...
	private void setCookieExam(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		  ...
		  * => 이외의 값(예를 들면 한글)을 사용시ㅔ는 URLEncoder.encode()를 사용하여 인코딩 처리를 해준다.
		  * 
		  ...
		  */
		 	...
		 	// 쿠키 소멸시간 설정(소단위) => 지정하지 않으면 웹브라우저를 종료할 때 쿠키를 함께 삭제한다.
