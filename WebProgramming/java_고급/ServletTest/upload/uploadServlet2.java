...
/**
 * 자카르타 프로젝트의 fileupload 모듈을 이욯한 파일업로드 예제
 */
				...
				if(formItems != null && formItems.size() > 0) {
					for(FileItem item : formItems) {
						if(!item.isFormField()) { // 파일인 경우...
							...
							// 폼필드의 값이 한글인 경우에는 해당 문자열을 적절히 변환을 해주어야한다.
							// 방법 1. value = new String(fileItem.getString().getBytes("8859-1"), "UTF-8");
							// 방법 2. value = fileItem.getString("UTF-8")
