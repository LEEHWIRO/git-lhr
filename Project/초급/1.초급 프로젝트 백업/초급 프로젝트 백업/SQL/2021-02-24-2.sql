...
--ORDERS와 INFO_ORDERS 외부조인
SELECT  A.ORDER_NO 주문번호,
        A.BUYER_ID 가맹점ID,
        A.ORDER_PRICE 주문가격,
        A.ORDER_MEMBER_DATE 고객주문날짜,
        A.ORDER_BUYER_CHOICE 가맹점승인날짜,
        B.INFO_ORDER_NO 주문정보번호,
        B.MENU_NO 메뉴분류코드,
        B.ORDER_NO 주문번호,
        B.INFO_CART_QTY 주문수량,
        C.MENU_NO 메뉴분류코드,
        C.MENU_GU 메뉴분류코드,
        C.MENU_GU_SEQ 메뉴분류코드시퀀스,
        C.MENU_NM 메뉴명,
        C.MENU_INGR 메뉴재료,
        C.MENU_PRICE 메뉴가격
FROM ORDERS A LEFT  OUTER JOIN INFO_ORDER B ON(A.ORDER_NO = B.ORDER_NO OR A.MEM_ID = 'DEMOMEMBER' AND A.ORDER_NO = '20210224090824')
                    INNER JOIN MENU C ON(B.MENU_NO = C.MENU_NO)
                    INNER JOIN ADD_INGR D ON(B.INFO_ORDER_NO = D.INFO_ORDER_NO)
                    INNER JOIN INGR E ON(E.INGR_NO = D.INGR_NO)
                    INNER JOIN BUYER F ON(A.BUYER_ID = F.BUYER_ID);