...
INSERT INTO ADD_INGR(ADD_INGR_NO, INFO_ORDER_NO, INGR_NO)
SELECT ADD_INGR_NO_SEQ.NEXTVAL, (SELECT INFO_ORDER_NO FROM INFO_ORDER), INGR_NO
FROM   CART_ADD_INGR A WHERE A.CART_NO = (SELECT B.CART_NO FROM CART B WHERE B.CART_NO = A.CART_NO);

UPDATE CART_ADD_INGR A
...
SELECT 111, (SELECT INFO_ORDER_NO FROM INFO_ORDER), INGR_NO
FROM   CART_ADD_INGR A 
WHERE A.CART_NO = (
        SELECT B.CART_NO 
        FROM   CART B 
        WHERE  B.CART_NO = A.CART_NO
    );
    
SELECT * FROM ADD_INGR;
...
FROM ORDERS A   LEFT OUTER JOIN BUYER B      ON(A.BUYER_ID = B.BUYER_ID) 
                LEFT OUTER JOIN INFO_ORDER C ON(A.ORDER_NO = C.ORDER_NO) 
                LEFT OUTER JOIN ADD_INGR D   ON(C.INFO_ORDER_NO = D.INFO_ORDER_NO) 
                LEFT OUTER JOIN INGR E       ON(D.INGR_NO = E.INGR_NO)  
                LEFT OUTER JOIN MENU F       ON(C.MENU_NO = F.MENU_NO 
AND A.MEM_ID = 'DEMOMEMBER1' 
AND   A.ORDER_NO = '20210224090824');
...
SELECT BUYER_ID
FROM   BUYER
WHERE  BUYER_NAME = '써브웨이 대전둔산점';

SELECT MENU_GU
FROM   MENU
WHERE  MENU_NM = '스테이크 아보카도 랩';
...
FROM ORDERS A   INNER JOIN BUYER B      ON(A.BUYER_ID = B.BUYER_ID) 
			     INNER JOIN INFO_ORDER C ON(A.ORDER_NO = C.ORDER_NO) 
                 INNER JOIN MENU D ON(C.MENU_NO = D.MENU_NO)
WHERE       A.ORDER_NO = '20210225042934';
...
FROM ORDERS A   INNER JOIN BUYER B      ON(A.BUYER_ID = B.BUYER_ID)
				INNER JOIN INFO_ORDER C ON(A.ORDER_NO = C.ORDER_NO)		
				INNER JOIN MENU F       ON(C.MENU_NO = F.MENU_NO)
WHERE A.ORDER_NO = '20210225043031'
AND   B.BUYER_ID = 'DEMOBUYER1';
